module CuadroFifa2026
  POSICIONES_TERCEROS = %w[A B D E G I K L].freeze

  DIECISEISAVOS = {
    73 => { local: [:pos, 2, "A"], visitante: [:pos, 2, "B"] },
    74 => { local: [:pos, 1, "E"], visitante: [:tercero, "E"] },
    75 => { local: [:pos, 1, "F"], visitante: [:pos, 2, "C"] },
    76 => { local: [:pos, 1, "C"], visitante: [:pos, 2, "F"] },
    77 => { local: [:pos, 1, "I"], visitante: [:tercero, "I"] },
    78 => { local: [:pos, 2, "E"], visitante: [:pos, 2, "I"] },
    79 => { local: [:pos, 1, "A"], visitante: [:tercero, "A"] },
    80 => { local: [:pos, 1, "L"], visitante: [:tercero, "L"] },
    81 => { local: [:pos, 1, "D"], visitante: [:tercero, "D"] },
    82 => { local: [:pos, 1, "G"], visitante: [:tercero, "G"] },
    83 => { local: [:pos, 2, "K"], visitante: [:pos, 2, "L"] },
    84 => { local: [:pos, 1, "H"], visitante: [:pos, 2, "J"] },
    85 => { local: [:pos, 1, "B"], visitante: [:tercero, "B"] },
    86 => { local: [:pos, 1, "J"], visitante: [:pos, 2, "H"] },
    87 => { local: [:pos, 1, "K"], visitante: [:tercero, "K"] },
    88 => { local: [:pos, 2, "D"], visitante: [:pos, 2, "G"] },
  }.freeze

  RONDAS_SIGUIENTES = {
    89 => { etapa: :octavos, local: [:ganador, 74], visitante: [:ganador, 77] },
    90 => { etapa: :octavos, local: [:ganador, 73], visitante: [:ganador, 75] },
    91 => { etapa: :octavos, local: [:ganador, 76], visitante: [:ganador, 78] },
    92 => { etapa: :octavos, local: [:ganador, 79], visitante: [:ganador, 80] },
    93 => { etapa: :octavos, local: [:ganador, 83], visitante: [:ganador, 84] },
    94 => { etapa: :octavos, local: [:ganador, 81], visitante: [:ganador, 82] },
    95 => { etapa: :octavos, local: [:ganador, 86], visitante: [:ganador, 88] },
    96 => { etapa: :octavos, local: [:ganador, 85], visitante: [:ganador, 87] },
    97 => { etapa: :cuartos, local: [:ganador, 89], visitante: [:ganador, 90] },
    98 => { etapa: :cuartos, local: [:ganador, 93], visitante: [:ganador, 94] },
    99 => { etapa: :cuartos, local: [:ganador, 91], visitante: [:ganador, 92] },
    100 => { etapa: :cuartos, local: [:ganador, 95], visitante: [:ganador, 96] },
    101 => { etapa: :semifinal, local: [:ganador, 97], visitante: [:ganador, 98] },
    102 => { etapa: :semifinal, local: [:ganador, 99], visitante: [:ganador, 100] },
    103 => { etapa: :tercer_lugar, local: [:perdedor, 101], visitante: [:perdedor, 102] },
    104 => { etapa: :final, local: [:ganador, 101], visitante: [:ganador, 102] },
  }.freeze
  ...
  def self.tercero_para(posicion, grupos_terceros)
    fila = ANNEX_C.fetch(grupos_terceros.sort.join)
    fila.fetch(posicion)
  end

  def self.consumidores(numero)
    RONDAS_SIGUIENTES.select do |_destino, plantilla|
      [plantilla[:local], plantilla[:visitante]].any? { |spec| spec[1] == numero }
    end.keys
  end
end
